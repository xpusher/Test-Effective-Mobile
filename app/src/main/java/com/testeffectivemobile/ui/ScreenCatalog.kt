...
fun ScreenCatalog(
    ...
    fun ShowCatalog(){
        ...
        val mockyContent= mutableMockyContent.collectAsStateWithLifecycle().value

        LazyVerticalGrid(columns = GridCells.Fixed(2)){
            items(count = mockyContent!!.items.length()) {
                Box(modifier = Modifier.aspectRatio(1f)) {
                    Text(text = mockyContent.item(it).pricePrice.toString())
                }
            }
        }
